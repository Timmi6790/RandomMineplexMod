package de.timmi6790.rmmod.modules.community.tabsupport;
...
import de.timmi6790.basemod.utilities.ListUtilities;
import de.timmi6790.basemod.values.bukkit.BukkitEffects;
import de.timmi6790.basemod.values.bukkit.BukkitMaterials;
import de.timmi6790.basemod.values.bukkit.BukkitMobs;
import de.timmi6790.basemod.values.mineplex.MineplexCosmetics;
import lombok.Getter;

import java.util.*;

public class EventCommandTabSupport extends AbstractTabSupport {
    private static final String[] SCOREBOARD_LINES = generateIntArray(1, 15);
    private static final String[] SCOREBOARD_CHAT_FORMATION_OPTIONS = {
            "&l", "&n", "&o", "&k", "&m", "&r", "&4", "&c", "&6", "&e",
            "&2", "&a", "&b", "&3", "&1", "&9", "&d", "&5", "&f", "&7"
    };

    private static final String[] SUGGESTED_MOB_NUMBERS = {"1", "32", "64", "128", "10000"};
    private static final String[] SUGGESTED_HEALTH = generateIntArray(1, 20);
    private static final String[] SUGGESTED_FOOD = generateIntArray(1, 20);
    ...
    public EventCommandTabSupport(final ModCache modCache) {
        super("/e");
    ...
    public void onTabSupport(final TabSupportData tabSupportData) {
        ...
        if (args.length == 2) {
            tabSupportData.setTabReturn(
                    DataUtilities.getStartWithIgnoreCase(
                            EnumUtilities.getPrettyNames(FirstParameter.values()),
                            args[1]
                    )
            );
            return;
    ...
    private void handleMobOptions(final TabSupportData tabSupportData, final String[] args) {
        if (args.length == 1) {
            final List<String> options = new ArrayList<>(EnumUtilities.getPrettyNames(BukkitMobs.values()));
            ...
            if (mobParameterOpt.isPresent() && mobParameterOpt.get() == MobParameter.KILL) {
                final List<String> options = EnumUtilities.getPrettyNames(BukkitMobs.values());
                options.add("all");
        ...
        final Optional<MobSpawnParameter> mobSpawnParameterOpt = EnumUtilities.getIgnoreCase(
                String.valueOf(lastArg.charAt(0)),
                MobSpawnParameter.values()
        );
        ...
        tabSupportData.setTabReturn(
                DataUtilities.combine(
                        lastArg,
                        this.getStartWithIgnoreCase(mobSpawnParameterOpt.get().getValues(),
                                lastArg.substring(1))
                )
        );
    }
    ...
    private void handleGadgetOptions(final TabSupportData tabSupportData, final String[] args) {
        if (args.length == 3) {
            final List<String> values = new ArrayList<>(Arrays.asList(MineplexCosmetics.getCosmetics()));
    ...
    private void handleTempGadgetOptions(final TabSupportData tabSupportData, final String[] args) {
        if (args.length == 3) {
            tabSupportData.setTabReturn(this.getStartWithIgnoreCase(MineplexCosmetics.getCosmetics(), args[2]));
        }
    }
    ...
    private void handleDamageOptions(final TabSupportData tabSupportData, final String[] args) {
        if (args.length == 3) {
            tabSupportData.setTabReturn(
                    DataUtilities.getStartWithIgnoreCase(
                            EnumUtilities.getPrettyNames(DamageParameter.values()),
                            args[2]
                    )
            );
        }
    ...
    private void handleBlockInteractionOptions(final TabSupportData tabSupportData, final String[] args) {
        switch (args.length) {
            ...
            case 4:
                tabSupportData.setTabReturn(
                        this.getStartWithIgnoreCase(
                                EnumUtilities.getPrettyNames(BlockSettingParameter.values()),
                                args[2]
                        )
                );
                return;
    ...
    private void handleEffectOptions(final TabSupportData tabSupportData, final String[] args) {
        if (args.length == 3) {
            ...
            final List<String> values = ListUtilities.toStringList(BukkitEffects.values(), Enum::name);
    ...
    private void handleKitOptions(final TabSupportData tabSupportData, final String[] args) {
        if (args.length == 3) {
            tabSupportData.setTabReturn(
                    DataUtilities.getStartWithIgnoreCase(
                            EnumUtilities.getPrettyNames(KitParameter.values()),
                            args[2]
                    )
            );
        }
    ...
    private enum MobSpawnParameter {
        ...
        I(ListUtilities.toStringList(BukkitMaterials.values(), Enum::name).toArray(new String[0])),
        E(ListUtilities.toStringList(BukkitEffects.values(), Enum::name).toArray(new String[0])),
