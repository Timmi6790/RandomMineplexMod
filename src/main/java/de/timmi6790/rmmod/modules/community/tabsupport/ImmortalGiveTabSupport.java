...
import de.timmi6790.basemod.values.bukkit.BukkitEnchantments;
...
public class ImmortalGiveTabSupport extends AbstractTabSupport {
    ...
    private static final String[] ENCHANTMENT_EXAMPLE = generateIntArray(1, 10);
    ...
    private void handlePlayerOptions(final String lastArg, final TabSupportData tabSupportData) {
        ...
        tabSupportData.setTabReturn(
                DataUtilities.combine(
                        prefix,
                        DataUtilities.getStartWithIgnoreCase(playersInLobby, lastPlayerName)
                )
        );
    }
    ...
    private void handleItemOptions(final String lastArg, final TabSupportData tabSupportData) {
        tabSupportData.setTabReturn(
                DataUtilities.getStartWithIgnoreCase(
                        DataUtilities.collectionToLowerCase(ListUtilities.toStringList(BukkitMaterials.values(), Enum::name)),
                        lastArg
                )
        );
    }
    ...
    private void handleEnchantOptions(final String lastArg, final TabSupportData tabSupportData) {
        ...
        if (lastEnchantPart.length == 1 && EnumUtilities.getIgnoreCase(lastEnchantPart[0], BukkitEnchantments.values()).isPresent()) {
            // Add level
            final Optional<BukkitEnchantments> enchantment = EnumUtilities.getIgnoreCase(lastEnchantPart[0], BukkitEnchantments.values());
            tabSupportData.setTabReturn(
                    DataUtilities.combine(
                            prefix + enchantment.get().name() + ":",
                            Arrays.asList(ENCHANTMENT_EXAMPLE)
                    )
            );
        } else {
            ...
            tabSupportData.setTabReturn(
                    DataUtilities.combine(
                            prefix,
                            DataUtilities.getStartWithIgnoreCase(enchantments, lastEnchantPart[0])
                    )
            );
        }
    }
